...
import VIRTUE_CONTENT from "@/lib/virtueContent";
import { useTheme, getPillStyle, getActionButtonStyle } from "@/lib/ThemeContext";

export const VIRTUE_DEFINITIONS: any = {
...
export const VIRTUE_CULTIVATE: any = {
...
export const VIRTUE_COLORS: any = {
  wisdom: "#d8b4fe",
  courage: "#fef08a",
  humanity: "#fda4af",
  justice: "#86efac",
...
export function getDailyItem(virtue: string, offset = 0) {
  const content = (VIRTUE_CONTENT as any)[virtue];
  if (!content) return null;
  const now = new Date();
  const dayOfYear = Math.floor((now.getTime() - new Date(now.getFullYear(), 0, 0).getTime()) / 86400000);
  const seed = dayOfYear * 100 + Object.keys(VIRTUE_CONTENT).indexOf(virtue) + offset * 1000;

  const items = content.pledges.map((p: any) => ({ ...p, type: "pledge" }));
  const idx = Math.floor(seededRandom(seed) * items.length);
  return items[idx];
}

export function getDailyChallenge(virtue: string, offset = 0) {
  const content = (VIRTUE_CONTENT as any)[virtue];
  if (!content) return null;
  const now = new Date();
  const dayOfYear = Math.floor((now.getTime() - new Date(now.getFullYear(), 0, 0).getTime()) / 86400000);
  ...
  const items = content.challenges.map((c: any) => ({ ...c, type: "challenge" }));
  const idx = Math.floor(seededRandom(seed) * items.length);
  return items[idx];
}
...
export default function VirtueCard({ virtue, isCompleted, onComplete, changeCount, onChange }: any) {
